...
const SkillProgression = () => {
  ...
  const getTotalStats = () => {
    ...
    const unlockedSkills = allSkills.filter(skill => skill.unlocked);
    const totalXp = unlockedSkills.reduce((sum, skill) => sum + skill.xp, 0);
    const maxXp = unlockedSkills.reduce((sum, skill) => sum + skill.maxXp, 0);
    ...
    return {
      ...
      totalXp,
      maxXp,
      completionRate: Math.round((unlockedSkills.length / allSkills.length) * 100)
    };
  };
      ...
      <section className="border-b border-border/50 bg-card/30">
        <div className="container mx-auto px-6 py-8">
          <div className="max-w-6xl mx-auto">
            <div className="mb-6">
              <p className="text-primary font-mono text-sm mb-2">SKILL DEVELOPMENT</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 font-mono text-sm mb-8">
              ...
              <div className="border-l-2 border-primary pl-4">
                <h3 className="text-foreground mb-1">TOTAL XP</h3>
                <p className="text-primary text-lg font-bold">{stats.totalXp.toLocaleString()}</p>
              </div>
              
              <div className="border-l-2 border-primary pl-4">
                <h3 className="text-foreground mb-1">AVERAGE LEVEL</h3>
                <p className="text-muted-foreground text-lg font-bold">2.3</p>
              </div>
            </div>
          </div>
        </div>
